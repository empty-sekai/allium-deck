...
- **Search**: SoA card pool with 512-bit metadata masks for fitting pools, power packed into u18×8 slots plus lookup tables, per-character dominance pruning, character-aware suffix upper bounds, and a greedy + 1-swap warm-start lower bound, so branch and bound prunes as early as possible.
...
Latency depends on candidate count, event rules, Top-K and hardware. Measure pool construction and search separately in the release profile, and report completion status alongside timings. Timed-out results do not establish complete Top-K.

The [validation guide](docs/search-validation.md) covers independent small-pool enumeration, full-result comparisons and reproducible timing measurements.
