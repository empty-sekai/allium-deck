...
No-event `score` exactness cost: without an event, the search keeps every candidate (no bonus-blind pool trimming), so on stress boxes with 1300 maxed cards the branch-and-bound enumerates ~1.65M near-optimal leaves. The scenario-aware upper bound is measured within 0.21% of the optimum at the root, so further admissible tightening cannot cut these ties — the remaining cost is the price of an exact Top-1 on the untrimmed pool (the C++ reference treats this scenario as heuristic and returns different decks across runs). Typical boxes search in milliseconds; the 215-event sweep runs measurably faster than the pre-fix engine.
