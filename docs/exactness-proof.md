...
only for the Score, Bonus and MySekai targets, when there are no fixed cards,
at most one fixed character, and bonus ordering is not observable; other
targets, multiple fixed slots or position-sensitive bonus semantics fall back
to the slot-aware DFS.
