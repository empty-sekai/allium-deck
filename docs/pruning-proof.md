...
The exact dense mask contains 8 × 64 = 512 card bits. If hard filtering leaves
more than 512 cards, the builder returns TooManyCards. Other compact metadata
limits are checked in the same way.
...
selected_card_ceiling_with_candidate_support_ub may additionally account for
the current candidate's exact support displacement. The current support value
is still an upper bound on all later extensions by Section 16.
