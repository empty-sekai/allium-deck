...
- **搜索（search）**：SoA 卡池 + 小池使用的 512-bit 元数据位图、综合力压成 u18×8 槽位 + 查找表、逐角色支配裁剪、角色感知后缀上界、贪心 + 1-swap warm start 下界，使分支限界尽早剪枝。
...
耗时取决于候选池大小、活动规则、Top-K 和硬件。请在 release profile 下分别记录建池与搜索耗时，同时报告完成状态；超时结果不代表完整 Top-K。
