...
fn main() {
    ...
    let packed = std::env::args().nth(2).expect("missing packed postcard path");
    let dir = PathBuf::from(&root);

    // 1) 原始路径：读 24 张 JSON + serde 解析 + from_sources 扁平化
    let t_read = Instant::now();
    let sources = MasterdataSources::from_dir(&dir, &PathBuf::from(
        "tests/music_metas.json",
    ))
    ...
    if let Some(sources) = sources {
        ...
        let restored: OwnedGameData =
            postcard::from_bytes(&bytes).expect("postcard decode failed");
        let decode_ms = t_decode.elapsed().as_millis() as f64;

        println!("cards={} events={} honors={}", owned.cards.len(), owned.events.len(), owned.honors.len());
        println!("restored cards={} (一致性校验: {})", restored.cards.len(), restored.cards.len() == owned.cards.len());
